...
def build_table(table_name: str, embedding_dimensions: int) -> type:
    ...
    Returns a cached class if the same table_name was already built.
    """
    if table_name in _table_cache:
        return _table_cache[table_name]

    class_name = f"MemoryRow_{table_name}"
    ...
    MemoryRow.__qualname__ = class_name
    MemoryRow.__name__ = class_name
    _table_cache[table_name] = MemoryRow
    return MemoryRow
